				  ...
				  unsigned long arg)
{
    ...
    DEBUGMSG("IOCTL CALLED");
    if(WRAPFS_IOCSETD == cmd){
        DEBUGMSG("COMMAND PASSED");
        user_key = (unsigned char*)arg;
        if(user_key == NULL){
            DEBUGMSG("NO KEY PASSED");
            return -EINVAL;
        }
        k_key = (unsigned char*)kmalloc(16, GFP_KERNEL);
        if(!k_key){
            DEBUGMSG("MEMMORY UNAVAILABLE");
            return -ENOMEM;
        }
        err = copy_from_user(k_key, user_key, 16);
        if(err < 0){
            DEBUGMSG("COPY FROM USER FAILED");
            goto out;
        }
        DEBUGMSG("KEY PASSED IS BELOW");
        DEBUGMSG(k_key);
        for(k_key_looper = 0; k_key_looper < 16; k_key_looper++){
            if(k_key[k_key_looper] != '0'){
                key_zeros = 1;
                break;
            }
        }
        if(key_zeros == 0){
            DEBUGMSG("PASSWORD REMOVED");
            sbi = (struct wrapfs_sb_info*)(file->f_path.dentry->d_sb->s_fs_info);
            kfree(sbi->sb_key);
            sbi->sb_key = NULL;
            if(sbi->sb_key == NULL){
                LOGMSG("NO KEY PRESENT");
                DEBUGMSG("KEY IS OBVIOUSLY REMOVED");
            }
            shrink_dcache_sb(file->f_path.dentry->d_sb);
            kfree(k_key);
            goto out;
        }
        ...
        sbi = (struct wrapfs_sb_info*)(file->f_path.dentry->d_sb->s_fs_info);
        if(sbi->sb_key == NULL){
            sbi->sb_key = (unsigned char*)kmalloc(16 ,GFP_KERNEL);
            memcpy(sbi->sb_key, k_key, 16);
            DEBUGMSG("KEY IN SUPERBLOCK IS BELOW");
            DEBUGMSG(sbi->sb_key);
            shrink_dcache_sb(file->f_path.dentry->d_sb);
            kfree(k_key);
        }
        else if(sbi->sb_key != NULL){
            DEBUGMSG("OLD KEY IS BELOW");
            DEBUGMSG(sbi->sb_key);
            memcpy(sbi->sb_key, k_key, 16);
            DEBUGMSG("NEW KEY IS BELOW");
            DEBUGMSG(sbi->sb_key);
            shrink_dcache_sb(file->f_path.dentry->d_sb);
            kfree(k_key);
        } 
